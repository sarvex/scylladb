...
class ASTBase:
    ...
    def ns_qualified_name(self):
        return (
            self.name
            if not self.ns_context
            else f"{self.combine_ns(self.ns_context)}::{self.name}"
        )
...
class Include(ASTBase):
    ...
    def __init__(self, name):
        ...
        if self.is_module:
            self.module_name = self.name[:-len(sfx)]
...
class RpcVerbParam(ASTBase):
    ...
    * Additional attributes (only [[version]] attribute is supported).
    ...
    def to_string_handle_ret_value(self):
        res = self.type.to_string()
        if self.is_unique():
            res = f'foreign_ptr<std::unique_ptr<{res}>>'
        elif self.is_lw_shared():
            res = f'foreign_ptr<lw_shared_ptr<{res}>>'
        return res
...
def template_params_str(template_params):
    if not template_params:
        return ""
    return ", ".join(
        map(lambda param: f"{param.typename} {param.name}", template_params)
    )
...
def element_type(t):
    ...
    assert t.name not in ["boost::variant", "std::variant"]
...
def add_view(cout, cls):
    ...
    for m in members:
        ...
        full_type = param_view_type(m.type)
        if m.attribute:
            deflt = m.default_value if m.default_value else f"{param_type(m.type)}()"
            if deflt in local_names:
                deflt = local_names[deflt]
